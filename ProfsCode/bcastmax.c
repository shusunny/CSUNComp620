...
#define mpc MPI_COMM_WORLD
 ...
 int getmax (int a[], int n, int id, int p){
    int i, k, max, startpt, endpt;
    ...
    int main(int argc, char **argv){
      ...
      // Compute my portion Of input	  
	   mymax = getmax(arr, n, id, p);

      // Compute global sum
      MPI_Reduce(&mymax, &finalmax, 1, MPI_INT, MPI_MAX, 0, mpc);
